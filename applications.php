...
<script>
            ...
            viewableCells.forEach(function(cell) {
                cell.addEventListener('click', function () {
                    ...
                    if (result.isConfirmed) {
                        ...
                        const remarksInput = document.querySelector('.swal2-textarea'); // Get the textarea element
                        const remarks = remarksInput ? remarksInput.value : ''; // Retrieve the value
    ...
    function updateStatus(id, status, remarks = '') {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        document.getElementById('row_' + id).cells[13].textContent = status;
        ...
        xhr.send("id=" + id + "&status=" + status + "&remarks=" + encodeURIComponent(remarks));
    }
</script>
