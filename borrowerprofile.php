  ...
  <div class="container">
    ...
    <form id="passwordForm" method="post">
      <div class="form-group">
        ...
        $name = '';
        if (!empty($email)) {
            $sql = "SELECT name FROM borrowersignup WHERE email = ?";
      ...
      <div class="error-message">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
          ...
          if (isset($_POST['oldPassword']) && isset($_POST['newPassword'])) {
              ...
              $storedPassword = '';
              if (!empty($email)) {
                  $sql = "SELECT password FROM borrowersignup WHERE email = ?";
                  $stmt = $conn->prepare($sql);
                  $stmt->bind_param("s", $email);
                  $stmt->execute();
                  $result = $stmt->get_result();
              ...
              if (password_verify($oldPassword, $storedPassword)) {
                  ...
                  if ($newPassword != $oldPassword && preg_match('/^(?=.*[A-Z])(?=.*[!@#$%^&*])(?=.*[0-9])(?=.{8,})/', $newPassword)) {
                      ...
                      $updateSql = "UPDATE borrowersignup SET password = ? WHERE email = ?";
